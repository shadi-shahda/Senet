...
//        System.out.println("Black possible actions (Expectimax evaluation):");
    ...
    private double chanceNode(Board board, int depth, Player player,SearchStatistic stats) {
        stats.visitedNodes++;
        stats.chanceNodes++;
        ...
        for (int roll = 1; roll <= 5; roll++) {
            ...
            expected += p * decisionNode(board, depth, player, roll,stats);
        }

        return expected;
    }

    private double decisionNode(Board board, int depth, Player player, int roll,SearchStatistic stats) {
        stats.visitedNodes++;
        if (player == Player.BLACK) stats.maxNodes++;
        else stats.minNodes++;
